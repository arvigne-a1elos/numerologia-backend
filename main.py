...
GOLD = colors.HexColor("#B8860B"); LGRAY = colors.HexColor("#f5f5f5"); DARK = colors.HexColor("#222")

SIG = {1:("Individualidade","Original, criativo, lider nato.","Egoista, arrogante.","Humildade."),2:("Associacao","Diplomatico, sensivel.","Indeciso.","Autoconfianca."),3:("Criacao","Criativo, comunicador.","Disperso.","Foco."),4:("Trabalho","Pratico, confiavel.","Rigido.","Flexibilidade."),5:("Liberdade","Livre, aventureiro.","Impulsivo.","Responsabilidade."),6:("Familia","Amoroso, protetor.","Superprotetor.","Confiar."),7:("Sabedoria","Sabio, analitico.","Frio.","Compartilhar."),8:("Poder","Realizador, prospero.","Materialista.","Integridade."),9:("Humanidade","Humanitario.","Melancolico.","Perdoar."),11:("Mestre Inspirador","Intuitivo.","Ansioso.","Equilibrar."),22:("Mestre Construtor","Realizador.","Ambicioso.","Escravizar.")}
CAM = {1:("Realizacao","Sua missao e liderar."),2:("Paz","Cooperar."),3:("Alegria","Criar."),4:("Acao","Construir."),5:("Evolucao","Mudar."),6:("Conciliacao","Servir."),7:("Sabedoria","Buscar."),8:("Justica","Prosperar."),9:("Humanitarismo","Servir."),11:("Inspiracao","Iluminar."),22:("Construcao","Realizar.")}
DES = {0:"Equilibrio.",1:"Superar egoismo.",2:"Timidez.",3:"Foco.",4:"Flexibilidade.",5:"Responsabilidade.",6:"Confiar.",7:"Compartilhar.",8:"Etica.",9:"Concluir."}
VIB = {1:"Lider nato.",2:"Sensivel.",3:"Criativo.",4:"Pratico.",5:"Livre.",6:"Amoroso.",7:"Sabio.",8:"Prospero.",9:"Generoso."}
...
def pdf8(data, name, bd):
    ...
    e.append(Paragraph("© A1ELOS", ParagraphStyle("F",fontSize=7,textColor=colors.HexColor("#999"),alignment=1,spaceBefore=15)))
    doc.build(e); return path

def pdf17(data, name, bd_str):
    path = f"/tmp/p17_{uuid.uuid4().hex[:8]}.pdf"
    doc = SimpleDocTemplate(path, pagesize=A4, leftMargin=45, rightMargin=45, topMargin=35, bottomMargin=35)
    e = []
    lp = data["life_path"]; kw, desc_cam = CAM.get(lp, ("", "")); nome_p = name.split()[0] if " " in name else name

    # PAG 1: CAPA + PERFIL + TABELA
    e.append(Spacer(1,30))
    e.append(Paragraph("MAPA NUMEROLOGICO", ParagraphStyle("TT",fontSize=24,textColor=GOLD,alignment=1,fontName="Helvetica-Bold",spaceAfter=4)))
    e.append(Paragraph("COMPLETO", ParagraphStyle("SU",fontSize=14,textColor=GOLD,alignment=1,fontName="Helvetica",spaceAfter=15)))
    e.append(Paragraph(name.upper(), ParagraphStyle("NM",fontSize=13,alignment=1,textColor=DARK)))
    e.append(Paragraph(bd_str, ParagraphStyle("DT",fontSize=10,alignment=1,textColor=colors.HexColor("#888"),spaceAfter=15)))
    ...
    tbl = Table(td, colWidths=[125,45,290])
    tbl.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),GOLD),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTSIZE",(0,0),(-1,-1),9),("GRID",(0,0),(-1,-1),0.5,colors.grey),("ALIGN",(1,0),(1,-1),"CENTER"),("BACKGROUND",(0,1),(-1,-1),LGRAY),("TEXTCOLOR",(0,1),(-1,-1),DARK),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("TOPPADDING",(0,0),(-1,-1),4),("BOTTOMPADDING",(0,0),(-1,-1),4)]))
    e.append(tbl)
    e.append(Spacer(1,10))
    e.append(Paragraph(f"<b>Seu Perfil:</b> Caminho de Vida {lp} ({kw}). Expressao {data['expression']}, Motivacao {data['soul_urge']}, Personalidade {data['personality']}, Destino {data['destiny']}.", ParagraphStyle("TX",fontSize=9,spaceAfter=4,leading=14,textColor=DARK)))
    e.append(Paragraph(f"<b>Caminho da Vida:</b> {desc_cam}", ParagraphStyle("TX",fontSize=9,spaceAfter=4,leading=14,textColor=DARK)))
    e.append(PageBreak())

    # PAG 2: ANALISE DETALHADA + CICLOS
    e.append(Paragraph("<b>Analise Detalhada</b>", ParagraphStyle("SE",fontSize=13,textColor=GOLD,fontName="Helvetica-Bold",spaceBefore=4,spaceAfter=8)))
    for k,l in [("life_path","Caminho de Vida"),("expression","Expressao"),("soul_urge","Motivacao da Alma"),("personality","Personalidade"),("destiny","Destino")]:
        ...
        e.append(Paragraph(f"<b>Positivo:</b> {pos}", ParagraphStyle("TX",fontSize=8.5,spaceAfter=1,leading=12,textColor=DARK)))
        e.append(Paragraph(f"<b>Negativo:</b> {neg}", ParagraphStyle("TX",fontSize=8.5,spaceAfter=1,leading=12,textColor=DARK)))
        e.append(Paragraph(f"<b>Licao:</b> {licao}", ParagraphStyle("TX",fontSize=8.5,spaceAfter=5,leading=12,textColor=DARK)))
    ...
    e.append(Paragraph("<b>Ciclos da Vida</b>", ParagraphStyle("SE",fontSize=13,textColor=GOLD,fontName="Helvetica-Bold",spaceBefore=6,spaceAfter=6)))
    e.append(Paragraph(f"<b>1 Formativo (0-{fe}a) Reg {c1n}:</b> Aprendizado.", ParagraphStyle("TX",fontSize=9,spaceAfter=3,leading=13,textColor=DARK)))
    e.append(Paragraph(f"<b>2 Produtivo ({fe+1}-{fe+27}a) Reg {c2n}:</b> Realizacao.", ParagraphStyle("TX",fontSize=9,spaceAfter=3,leading=13,textColor=DARK)))
    e.append(Paragraph(f"<b>3 Colheita ({fe+28}+a) Reg {c3n}:</b> Sabedoria.", ParagraphStyle("TX",fontSize=9,spaceAfter=3,leading=13,textColor=DARK)))
    e.append(PageBreak())
    ...
    e.append(Paragraph("<b>Desafios da Vida</b>", ParagraphStyle("SE",fontSize=13,textColor=GOLD,fontName="Helvetica-Bold",spaceBefore=4,spaceAfter=6)))
    ...
    e.append(Paragraph("<b>Realizacoes</b>", ParagraphStyle("SE",fontSize=13,textColor=GOLD,fontName="Helvetica-Bold",spaceBefore=4,spaceAfter=6)))
    e.append(Paragraph(f"<b>1 ({r1v}):</b> Juventude.  <b>2 ({r2v}):</b> Vida adulta.  <b>3 ({r3v}):</b> Maturidade.  <b>4 ({r4v}):</b> Legado.", ParagraphStyle("TX",fontSize=9,spaceAfter=8,leading=13,textColor=DARK)))
    vib = r1(d)
    e.append(Paragraph("<b>Vibracao do Nascimento</b>", ParagraphStyle("SE",fontSize=13,textColor=GOLD,fontName="Helvetica-Bold",spaceBefore=4,spaceAfter=6)))
    e.append(Paragraph(f"Dia {bb.day}, vibracao {vib}. {VIB.get(vib,'')}", ParagraphStyle("TX",fontSize=9,spaceAfter=8,textColor=DARK)))
    e.append(Paragraph("<b>Grade de Inclusao</b>", ParagraphStyle("SE",fontSize=13,textColor=GOLD,fontName="Helvetica-Bold",spaceBefore=4,spaceAfter=6)))
    grid = calc_grid(name)
    presentes = [str(n) for n in range(1,10) if grid.get(n,0) > 0]
    ausentes = [str(n) for n in range(1,10) if grid.get(n,0) == 0]
    e.append(Paragraph(f"<b>Presentes:</b> {', '.join(presentes) if presentes else 'nenhum'}. <b>Carencias:</b> {', '.join(ausentes) if ausentes else 'nenhum'}.", ParagraphStyle("TX",fontSize=9,spaceAfter=8,textColor=DARK)))
    e.append(Paragraph("<b>Nota Final</b>", ParagraphStyle("SE",fontSize=13,textColor=GOLD,fontName="Helvetica-Bold",spaceBefore=4,spaceAfter=6)))
    e.append(Paragraph("A numerologia ilumina caminhos e revela potencialidades. Os numeros mostram tendencias, mas o livre arbitrio sempre prevalece.", ParagraphStyle("TX",fontSize=9,spaceAfter=12,textColor=DARK)))
    e.append(Paragraph("© A1ELOS Assessoria e Consultoria", ParagraphStyle("FF",fontSize=7,textColor=colors.HexColor("#999"),alignment=1)))
    doc.build(e); return path
