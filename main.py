...
TAM_T = 20; TAM_C = 14; EL = TAM_C * 1.5; ET = TAM_T * 2.0
...
def pdf_eleitoral(ss, cl, sugestoes, ne=None):
    ...
    e.append(Paragraph("O numero 8 representa Poder, Prosperidade e Realizacao material. Para candidatos politicos, atrai autoridade, sucesso nas urnas e capacidade de realizar grandes obras. Politicos como Henry Ford, Silvio Santos e Getulio Vargas possuem o 8 como numero de expressao.", J))
    ...
    if fbs:
        ...
        e.append(Paragraph("<b>Opcoes Alternativas (caso o ideal nao esteja disponivel):</b>", ParagraphStyle("B", fontName=FN, fontSize=TAM_C-1, leading=EL*0.95, textColor=DARK, spaceAfter=EL*0.3)))
        for s in fbs:
    ...
    e.append(Paragraph("Atencao: Verifique a disponibilidade do numero com seu partido antes de escolher. A prioridade de uso e de quem ja concorreu com aquele numero por antiguidade na sigla.", ParagraphStyle("AV", fontName=FONTE, fontSize=TAM_C-2, leading=EL*0.7, textColor=GRAY, spaceAfter=EL)))
...
def pay_urna_session(req: UrnaPayReq):
    ...
    cs = stripe.checkout.Session.create(mode="payment",payment_method_types=["card"],
        ...
        success_url=f"{BASE_URL}/api/pay/urna-success?session_id={{CHECKOUT_SESSION_ID}}",cancel_url=f"{BASE_URL}/api/pay/cancel")
