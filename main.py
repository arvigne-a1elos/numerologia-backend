...
SIG = {
    1: ("Individualidade", "Original, criativo, lider nato.", "Egoista, arrogante.", "Humildade."),
    2: ("Associacao", "Diplomatico, sensivel.", "Indeciso.", "Autoconfianca."),
    3: ("Criacao", "Criativo, comunicador.", "Disperso.", "Foco."),
    4: ("Trabalho", "Pratico, confiavel.", "Rigido.", "Flexibilidade."),
    5: ("Liberdade", "Livre, aventureiro.", "Impulsivo.", "Responsabilidade."),
    ...
    9: ("Humanidade", "Humanitario.", "Melancolico.", "Perdoar."),
    11: ("Mestre Inspirador", "Intuitivo.", "Ansioso.", "Equilibrar."),
    22: ("Mestre Construtor", "Realizador.", "Ambicioso.", "Escravizar.")}
...
def pdf17(data, name, bd_str):
    ...
    e.append(Paragraph(f"<b>Seu Perfil:</b> Caminho de Vida {lp} ({kw}). Expressao {data['expression']}, Motivacao {data['soul_urge']}, Personalidade {data['personality']}, Destino {data['destiny']}.", ParagraphStyle("TX", fontSize=9.5, spaceAfter=6, leading=15, textColor=DARK)))
    ...
    e.append(Paragraph(f"<b>2 Produtivo ({fe+1}-{fe+27}a) Reg {c2n}:</b> Realizacao.", ParagraphStyle("TX", fontSize=9.5, spaceAfter=4, leading=14, textColor=DARK)))
    ...
    d2=r1(abs(m-r1(aa)))
    dp_=r1(abs(d1-d2))
    e.append(Paragraph("<b>Desafios</b>", ParagraphStyle("SE", fontSize=13, textColor=GOLD, fontName="Helvetica-Bold", spaceBefore=8, spaceAfter=8)))
    ...
    e.append(Paragraph("A numerologia ilumina caminhos e revela potencialidades. Os numeros mostram tendencias, mas o livre arbitrio sempre prevalece.", ParagraphStyle("TX", fontSize=9.5, spaceAfter=15, textColor=DARK)))
...
def pay_success(request: Request):
    ...
    try:
        ...
        total = getattr(s,'amount_total',0) or getattr(s,'amount_subtotal',0) or 0
        total = int(total)
        logger.info(f"Produto={prod} total_cents={total}")
        if prod == 'pdf17' or total >= 1200:
            product = 'pdf17'
        else:
            product = 'pdf8'
        logger.info(f"Gerando: {product}")
    except Exception as e:
