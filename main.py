...
def pdf8(data, name, bd):
    ...
    for k,l in [("life_path","Cam.Vida"),("expression","Expressao"),("soul_urge","Mot.Alma"),
        ...
        v = data[k]; e.append(Paragraph(f"<b>{l} {v}:</b> {txt.get(v,'Unico.')}", ParagraphStyle("D",fontSize=9,spaceAfter=4,textColor=colors.white)))
    e.append(Spacer(1,20))
    e.append(Paragraph("© A1ELOS", ParagraphStyle("F",fontSize=7,textColor=colors.HexColor("#666"),alignment=1)))
    doc.build(e); return path

SIG = {1:("Individualidade","Original, criativo.","Egoista.","Humildade."),2:("Associacao","Diplomatico.","Indeciso.","Confianca."),
       3:("Criacao","Criativo.","Disperso.","Foco."),4:("Trabalho","Pratico.","Rigido.","Flexibilidade."),
       5:("Liberdade","Aventureiro.","Impulsivo.","Responsabilidade."),6:("Familia","Amoroso.","Superprotetor.","Confiar."),
       7:("Sabedoria","Sabio.","Frio.","Equilibrar."),8:("Poder","Realizador.","Materialista.","Integridade."),
       9:("Humanidade","Humanitario.","Melancolico.","Perdoar."),11:("Intuitivo","Inspirador.","Ansioso.","Equilibrar."),22:("Construtor","Visionario.","Excesso.","Construir.")}
...
DES = {0:"Fluxo.",1:"Egoismo.",2:"Timidez.",3:"Foco.",4:"Flexibilidade.",5:"Responsabilidade.",6:"Confiar.",7:"Isolamento.",8:"Etica.",9:"Concluir."}
...
def pdf17(data, name, bd_str):
    ...
    e.append(Paragraph(f"<b>Menor1 (DxM) {d1}:</b> {DES.get(d1,'')}", sd))
    e.append(Paragraph(f"<b>Menor2 (MxA) {d2}:</b> {DES.get(d2,'')}", sd))
    ...
    e.append(Paragraph("Frequencia dos numeros no nome. Consulte Mapa Premium.", sd))
    e.append(Spacer(1,10))
...
def pay_success(request: Request):
    ...
    try:
        data = calc(name, bd)
        if product == 'pdf17':
            pf = pdf17(data, name, bd); subj = "Seu Mapa Completo!"
        else:
